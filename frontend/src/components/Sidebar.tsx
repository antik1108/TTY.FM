    ...
    <>
      ...
      >
        <div className="bg-terminal-border/40 px-4 py-2 flex justify-between items-center text-[11px] uppercase font-bold text-neon-purple border-b-2 border-terminal-border crt-glow-purple">
          ...
          <span className="material-symbols-outlined text-xs cursor-pointer hidden lg:block">
            close_fullscreen
          </span>
        </div>
          ...
          <nav className="space-y-2 text-[12px]">
            ...
            >
              ...
              <span className="text-[10px] text-gray-500">
                [
                {playlists.reduce((sum, pl) => sum + pl.songCount, 0) +
                  uncategorizedCount}
                ]
              </span>
            </button>
            ...
            <div className="mt-4 text-[9px] text-neon-purple font-bold uppercase tracking-tighter">
              Playlists
            </div>
            {playlists.map((pl) => {
              const isActive =
                viewMode === ViewMode.PLAYLIST && selectedPlaylistId === pl.id;
              return (
                <button
                  key={pl.id}
                  onClick={() => {
        ...
        <div className="p-4 border-t-2 border-terminal-border bg-black space-y-3">
          <div className="text-[9px] text-neon-purple font-bold crt-glow-purple uppercase tracking-tighter">
            I/O Console
          </div>
          <div className="flex items-center gap-2 bg-terminal-border/40 px-3 py-2.5 rounded border border-terminal-border">
            <span className="text-matrix-green font-bold crt-glow-green">
              $
            </span>
            <div className="flex items-center flex-1 overflow-hidden">
              <span className="text-[12px] text-cyber-cyan truncate">
                scan --library
              </span>
