    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="bg-terminal-border/40 px-4 py-2 flex justify-between items-center text-[11px] uppercase font-bold text-neon-purple border-b-2 border-terminal-border crt-glow-purple sticky top-0 z-20 h-[42px]">
        <div className="flex items-center gap-2 flex-1">
          {isEditing ? (
            ...
            <>
              <span>
                {isPlaylist ? 'Playlist: ' : ''}
                <span className={isPlaylist ? 'text-white' : ''}>
                  {displayTitle}
                </span>
              </span>
              {isPlaylist && onRename && (
        ...
        <div className="flex items-center gap-6">
          ...
          <span className="material-symbols-outlined text-xl cursor-pointer hover:text-white transition-colors">
            tune
          </span>
          <span className="material-symbols-outlined text-xl cursor-pointer hover:text-white transition-colors">
            search
          ...
          <tbody className="divide-y divide-terminal-border/20">
            ...
            {error && !isLoading && (
              <tr>
                <td
                  colSpan={4}
                  className="px-6 py-10 text-center text-alert-yellow"
                >
                  {error}
                </td>
              </tr>
            ...
            {!isLoading &&
              ...
              songs.map((song) => {
                ...
                return (
                  ...
                  >
                    <td
                      className={`px-6 py-5 font-mono ${isActive ? 'text-white' : 'text-alert-yellow'}`}
                    >
                      {song.hexId}
                    </td>
                    <td className="px-6 py-5">
                      <div
                        className={`font-bold ${isActive ? 'crt-glow-purple' : ''}`}
                      >
                        {song.title}
                      </div>
                    ...
                    <td
                      className={`px-6 py-5 hidden md:table-cell ${isActive ? 'text-neon-purple font-black' : 'text-gray-400'}`}
                    >
                      {song.genre || 'AUDIO'}
                    </td>
                    <td
                      className={`px-6 py-5 text-right font-mono ${isActive ? 'text-white' : 'text-gray-500'}`}
                    >
                      {song.size || '--'}
                    </td>
                  </tr>
